...
CFG = {
    ...
    # 120 NO alcanzaba para confirm.py: pedía 120 retornos mínimos y solo le
    # llegaban 99, así que conf_regimen salía siempre "pocas velas (99)" y la
    # columna de régimen del CSV no valía para nada a los 15 días.
    "KLINES_LIMIT": env("KLINES_LIMIT", 260),
    # Espaciado mínimo entre muestras guardadas. Sin esto, el tamaño de la
    # historia lo fija SCAN_SEC: al bajarlo de 300 a 120 las series pasan de
    # ~1000 a ~4000 puntos por símbolo, el estado se va a decenas de MB y el
    # cálculo del z de OI se dispara. Con 300 s la historia mide lo mismo
    # (168 h) independientemente de lo rápido que gire el ciclo.
    "MUESTRA_MIN_SEG": env("MUESTRA_MIN_SEG", 300.0),
    "MAX_PUNTOS": env("MAX_PUNTOS", 2200),    # tope duro por serie
    # Cerilla: estricta = cierre fuera del rango de la vela anterior.
...
def prima(symbol: str):
    """Respaldo por símbolo cuando la llamada global no trae el símbolo."""
    ...
    try:
        ...
        return {"mark": mark, "index": index,
                "basis": (mark - index) / index * 100.0, "funding": fr * 100.0}
    except (TypeError, ValueError):
        return None
...
def _fetch_symbol_raw(symbol: str, con_prima: bool = False) -> dict | None:
    """Worker: descarga OI + klines (+ prima si la global no la trajo)."""
    try:
        pac = float(CFG["PACING"])
        if pac > 0:
            time.sleep(pac)
        ...
        if oi is None or not velas:
            return None
        out = {"oi": oi, "velas": velas}
        if con_prima:
            out["prima"] = prima(symbol)
        return out
    except Exception:
...
def _cambios_oi(lo: list, look_h: float) -> list[float]:
    ...
    La versión anterior copiaba la deque entera en cada iteración y hacía
    min() sobre ella. Medido: con 4000 puntos por símbolo (los que salen con
    SCAN_SEC=120 y 168 h de retención) son 565 ms POR SÍMBOLO, o sea ~170 s
    de CPU por ciclo con 300 símbolos, más que el propio SCAN_SEC. Aquí es
...
def _apuntar(h: deque, ahora: float, valor: float, min_seg: float, tope: int):
    """
    Guarda la muestra solo si ha pasado min_seg desde la última. El valor
    ACTUAL no se pierde: se usa siempre para el z, se guarde o no. Lo único
    que se controla aquí es cuánta historia se acumula.
    """
...
def evaluar(symbol: str, velas: list, p: dict, oi: float, st: Estado):
    ...
    # La muestra se apunta ANTES de cualquier descarte por velas: si un símbolo
    # se salta el ciclo, su serie se queda con un agujero y el z posterior se
    # calcula sobre otra cosa.
    ahora = time.time()
    hb = st.basis.setdefault(symbol, deque())
    ho = st.oi.setdefault(symbol, deque())
    min_seg = float(CFG["MUESTRA_MIN_SEG"])
    tope = int(CFG["MAX_PUNTOS"])
    _apuntar(hb, ahora, p["basis"], min_seg, tope)
    _apuntar(ho, ahora, oi, min_seg, tope)
    ...
    # confirm.py necesita CONFIRM_WIN+1 cierres, no 100: con 100 le llegaban
    # 99 retornos y devolvía "pocas velas" siempre.
    ...
    # La cerilla: primera vela EN CONTRA de la multitud. Estricta exige cerrar
    # fuera del rango de la vela anterior (envolvente), y además se evalúa
    # sobre la vela EN CURSO. Suelta pide solo vela contraria que pierda el
    # cierre anterior.
    ult, ant = velas[-1], velas[-2]
    if bool(CFG["TRIG_ESTRICTO"]):
...
# Embudo acumulado desde el arranque. El recuento de un solo ciclo no dice si
# el amontonamiento llega a dispararse ALGUNA vez: si "esperando vela en
# contra" sigue a 0 después de días, el cuello son los z; si sube y no pasa de
# ahí, el cuello es la cerilla.
EMBUDO: dict[str, int] = {}
...
def ciclo(st: Estado, simbolos: list[str]):
    ...
    #    Si la llamada global falla NO se aborta el ciclo: abortar significa
    #    no apuntar ninguna muestra, y el calentamiento se queda congelado sin
    #    que nada lo indique salvo una línea de warning.
    premiums = premium_todos()
    if not premiums:
        log.warning("premiumIndex global vacío: se pide símbolo a símbolo")
    faltan = [s for s in simbolos if s not in premiums]

    # 2) Descarga paralela de OI + klines (+ prima de respaldo si falta)
    raw: dict[str, dict] = {}
    workers = max(1, int(CFG["MAX_WORKERS"]))
    faltan_set = set(faltan)
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futs = {ex.submit(_fetch_symbol_raw, sym, sym in faltan_set): sym for sym in simbolos}
    ...
    sin_datos = len(simbolos) - len(raw)
    ...
    for sym in simbolos:
        try:
            ...
            velas = data["velas"]
            oi = data["oi"]
            p = premiums.get(sym) or data.get("prima")

            seguir_virtuales(st, sym, velas)

            # evaluar() se llama SIEMPRE, también con virtual abierta: es quien
            # apunta la muestra de basis y OI. Saltárselo deja agujeros en la
            # historia justo en los símbolos que están operando.
            sig, motivo = evaluar(sym, velas, p, oi, st)
            clave = motivo.split("(")[0].strip()
            razones[clave] = razones.get(clave, 0) + 1
            motivos += 1
            if sig is None or sym in st.abiertas:
                continue
    ...
    if señales > 0 or calentando == 0:
        log.info("Ciclo: %d símbolos · %d señales · cadencia %.1f min | workers=%d | "
                 "listos %d/%d · sin datos %d · %s",
                 motivos, señales, cad, workers, listos, total, sin_datos,
                 " · ".join(f"{k}: {v}" for k, v in top))
    else:
        log.info("Ciclo: %d símbolos · 0 señales · cadencia %.1f min | calentando %d/%d "
                 "(media %.1f h, %.0f muestras) | sin datos %d | %s",
                 motivos, cad, calentando, total, media_h, media_n, sin_datos,
                 " · ".join(f"{k}: {v}" for k, v in top[:3]))
    # TODAS las razones acumuladas, no las 4 del ciclo: es lo único que dice
    # dónde se corta el embudo cuando el resultado lleva días siendo 0.
...
def main():
    ...
    log.info("Filtros: Z_BASIS=%s Z_OI=%s EXT_PCT=%s · cerilla=%s · muestra cada %ss "
             "(tope %s puntos, retención %sh) · klines=%s",
             CFG["Z_BASIS"], CFG["Z_OI"], CFG["EXT_PCT"],
             "estricta" if CFG["TRIG_ESTRICTO"] else "suelta",
             CFG["MUESTRA_MIN_SEG"], CFG["MAX_PUNTOS"], CFG["HIST_HORAS"],
             CFG["KLINES_LIMIT"])
