...
class Config:
    ...
    rr:          float = 2.0    # Pine: tp_mult=2.0 → TP = (close-sl) × 2.0

    # ── Filtros ───────────────────────────────────────────────
    min_atr_pct:  float = 0.0   # Pine Script no filtra por ATR%
    min_vol_mult: float = 1.5   # Pine: vol_mult=1.5 (institucional)
    min_score:    int   = 2     # Pine: 3 condiciones → score=2 base (sin 15m)
