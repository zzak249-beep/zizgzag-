...
class PositionManager:
    ...
    async def register_trade(self, trade: OpenTrade, symbol: str = ""):
        async with self._lock:
    ...
    async def _move_to_breakeven(self, trade: OpenTrade, current_price: float, real_map: dict = None):
        ...
        Usa real_map del ciclo para no hacer llamada extra a BingX.
        """
        try:
            # Verificar con real_map del ciclo actual (sin llamada extra)
            if real_map is not None and trade.symbol not in real_map:
                log.info("[%s] BE skip — no en real_map del ciclo", trade.symbol)
