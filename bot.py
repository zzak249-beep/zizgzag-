...
# BingX mezcla en el mismo universo USDT-M cripto real con acciones y
# materias primas tokenizadas (AAPLX, NVDAX, TSLAX, COINX, METAX, HOODX,
# CRCLX... y contratos con prefijo NCS/NCCO respaldados por proveedores
# de liquidez institucionales). Confirmado con investigacion real: estos
# instrumentos pueden mostrar "Market closed" (no cotizan 24/7 de verdad
# como cripto) -- la estrategia entera (kill zones, dinamica de sweep)
# esta calibrada para liquidez cripto continua, no para esto. Encontrado
# en logs de produccion: NCSKFLNC2USD, NCSKNOK2USD, NCCO7241NATGAS2USD,
    ...
    def get_symbols(self) -> list:
        ...
        for c in rows:
            ...
            if sym.upper() in SYMBOL_BLACKLIST:
                continue
            if EXCLUDE_TOKENIZED_ASSETS:
                base = sym.upper().split("-")[0]
                if base.startswith(_TOKENIZED_PREFIXES) or base in _TOKENIZED_STOCK_BASES:
                    continue
...
_active_days: set = set()  # dias distintos (YYYY-MM-DD, hora local) con al menos una apertura --
                            # cripto se mueve correlacionado: 714 operaciones en 4 dias distintos
                            # no son 714 pruebas independientes de la estrategia, son 4 movimientos
                            # de mercado replicados en muchos simbolos a la vez.
    ...
    try:
        with open(STATE_FILE, "w") as f:
            json.dump({
                ...
                "active_days": sorted(_active_days),
            }, f)
    except Exception as exc:
...
def _load_paper_state() -> None:
    ...
    try:
        ...
        _active_days.update(data.get("active_days", []))
        log.info(f"Estado de papel restaurado desde {STATE_FILE}: {len(_paper_positions)} abiertas, "
                 f"{state['paper_wins']}W/{state['paper_losses']}L, {len(_active_days)} dias distintos")
...
def _dispatch_signal(s: dict, open_symbols: set) -> None:
    ...
    if DRY_RUN:
        ...
        state["paper_open"] = len(_paper_positions)
        _active_days.add(time.strftime("%Y-%m-%d"))
        _save_paper_state()
        return
...
def run_cycle() -> None:
    ...
    if DRY_RUN:
        ...
        avg_per_day = wl_total / len(_active_days) if _active_days else 0
        log.info(
            f"Papel: {state['paper_open']} abiertas | "
            f"{state['paper_wins']}W/{state['paper_losses']}L "
            f"({_paper_winrate():.0f}% de {wl_total} cerradas en {len(_active_days)} dias distintos, "
            f"~{avg_per_day:.0f}/dia)"
        )
