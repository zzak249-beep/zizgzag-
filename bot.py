...
async def enter_trade(sig) -> None:
    ...
    # Mínimo 0.01% de distancia SL (evita SL casi en precio)
    if abs(sig.price - sig.sl) / sig.price * 100 < 0.01: return
...
async def main_loop() -> None:
    ...
    logger.info("=" * 65)
    logger.info("  PHANTOM EDGE BOT v6.2 — ZigZag Institutional Elite V6")
    logger.info(f"  Pivot:{cfg.pivot_len} VolMult:{cfg.min_vol_mult}x RR:1:{cfg.rr}")
    logger.info(f"  SL=Valley/Peak  TP=SL×{cfg.rr}  Score≥{cfg.min_score}")
